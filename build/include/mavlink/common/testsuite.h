...
static void mavlink_test_ipc_data(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
    mavlink_status_t *status = mavlink_get_channel_status(MAVLINK_COMM_0);
        if ((status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) && MAVLINK_MSG_ID_IPC_DATA >= 256) {
            return;
        }
#endif
    mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
    mavlink_ipc_data_t packet_in = {
        963497464,{ 45.0, 46.0, 47.0, 48.0, 49.0, 50.0, 51.0, 52.0, 53.0, 54.0, 55.0, 56.0, 57.0, 58.0, 59.0, 60.0, 61.0, 62.0, 63.0, 64.0 },605.0,633.0,661.0,37,104,171,238
    };
    mavlink_ipc_data_t packet1, packet2;
        memset(&packet1, 0, sizeof(packet1));
        packet1.preamble = packet_in.preamble;
        packet1.steer_fb = packet_in.steer_fb;
        packet1.acc_fb = packet_in.acc_fb;
        packet1.ebrake_fb = packet_in.ebrake_fb;
        packet1.mode = packet_in.mode;
        packet1.mode_fb = packet_in.mode_fb;
        packet1.switch_fb = packet_in.switch_fb;
        packet1.hbrake_fb = packet_in.hbrake_fb;
        
        mav_array_memcpy(packet1.refrences, packet_in.refrences, sizeof(float)*20);
        
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
        if (status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) {
           // cope with extensions
           memset(MAVLINK_MSG_ID_IPC_DATA_MIN_LEN + (char *)&packet1, 0, sizeof(packet1)-MAVLINK_MSG_ID_IPC_DATA_MIN_LEN);
        }
#endif
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_ipc_data_encode(system_id, component_id, &msg, &packet1);
    mavlink_msg_ipc_data_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_ipc_data_pack(system_id, component_id, &msg , packet1.preamble , packet1.mode , packet1.refrences , packet1.mode_fb , packet1.switch_fb , packet1.steer_fb , packet1.acc_fb , packet1.ebrake_fb , packet1.hbrake_fb );
    mavlink_msg_ipc_data_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_ipc_data_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.preamble , packet1.mode , packet1.refrences , packet1.mode_fb , packet1.switch_fb , packet1.steer_fb , packet1.acc_fb , packet1.ebrake_fb , packet1.hbrake_fb );
    mavlink_msg_ipc_data_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
    ...
    mavlink_msg_ipc_data_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
        
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_ipc_data_send(MAVLINK_COMM_1 , packet1.preamble , packet1.mode , packet1.refrences , packet1.mode_fb , packet1.switch_fb , packet1.steer_fb , packet1.acc_fb , packet1.ebrake_fb , packet1.hbrake_fb );
    mavlink_msg_ipc_data_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

#ifdef MAVLINK_HAVE_GET_MESSAGE_INFO
    MAVLINK_ASSERT(mavlink_get_message_info_by_name("IPC_DATA") != NULL);
    MAVLINK_ASSERT(mavlink_get_message_info_by_id(MAVLINK_MSG_ID_IPC_DATA) != NULL);
#endif
}
...
static void mavlink_test_common(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
    ...
    mavlink_test_ipc_data(system_id, component_id, last_msg);
}
